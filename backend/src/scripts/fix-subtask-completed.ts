...
async function fixSubtaskCompleted() {
  try {
    ...
    await db.execute(
      sql`ALTER TABLE subtasks ALTER COLUMN completed DROP DEFAULT;`
    );
    
    // Step 2: Convert the column type
    console.log('Step 2: Converting column type...');
    await db.execute(
      sql`ALTER TABLE subtasks ALTER COLUMN completed SET DATA TYPE boolean USING (completed::text::boolean);`
    );
    
    // Step 3: Re-add the default value
    console.log('Step 3: Re-adding default value...');
    await db.execute(
      sql`ALTER TABLE subtasks ALTER COLUMN completed SET DEFAULT false;`
    );
