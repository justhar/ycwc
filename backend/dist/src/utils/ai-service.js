...
class AIService {
    ...
    async analyzeCVFile(fileBuffer, mimeType) {
        try {
...
6. If a field is not found, omit it from the JSON or set it to null`,
            ...
            return {
                success: false,
                error: error instanceof Error ? error.message : "AI file analysis failed",
            };
        }
    }
    createProfileExtractionPrompt(cvText) {
        return `
...
`;
    ...
    parseAIResponse(response) {
        try {
            ...
            if (parsed.targetLevel &&
                ["undergraduate", "graduate", "postgraduate"].includes(parsed.targetLevel)) {
            ...
            if (Array.isArray(parsed.englishTests)) {
                cleanedData.englishTests = parsed.englishTests.filter((test) => test &&
                    typeof test.type === "string" &&
                    typeof test.score === "string");
            }
            if (Array.isArray(parsed.standardizedTests)) {
                cleanedData.standardizedTests = parsed.standardizedTests.filter((test) => test &&
                    typeof test.type === "string" &&
                    typeof test.score === "string");
            }
            if (Array.isArray(parsed.awards)) {
                cleanedData.awards = parsed.awards.filter((award) => award && typeof award.title === "string");
            }
            if (Array.isArray(parsed.extracurriculars)) {
                cleanedData.extracurriculars = parsed.extracurriculars.filter((activity) => activity && typeof activity.activity === "string");
            }
            return cleanedData;
        }
        catch (error) {
    ...
    async matchUniversities(userProfile, universities) {
...
- Academic Score: ${userProfile.academicScore || "Not provided"} (Scale: ${userProfile.scoreScale || "Not specified"})
- Nationality: ${userProfile.nationality || "Not specified"}
- Intended Study Abroad Country: ${userProfile.intendedCountry || "Not specified"}
...
${userProfile.extracurriculars
                ?.map((activity) => `- ${activity.activity} (${activity.period}): ${activity.description || ""}`)
...
- acceptanceRate must be string like "65.50"`;
            ...
            try {
                ...
                console.error("Cleaned response sample:", matchResponseText.substring(0, 500));
                parsedMatches = { matches: [] };
            }
            // Process matches
            const processedMatches = parsedMatches.matches
                ?.map((match) => {
                const university = universities.find((uni) => uni.id === match.universityId);
                return {
                    university,
                    matchScore: typeof match.matchScore === "string"
  ...
  "suggestions": [
    {
      ...
      "website": "string"
                ...
                try {
                    ...
                    if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
                        cleanedSuggest = cleanedSuggest.substring(firstBrace, lastBrace + 1);
                    }
                    ...
                    suggestedUniversities = (parsedSuggest.suggestions || []).map((uni) => ({
                        ...uni,
                        ranking: typeof uni.ranking === "string"
                            ? parseInt(uni.ranking)
                            : uni.ranking,
                        studentCount: typeof uni.studentCount === "string"
                            ? parseInt(uni.studentCount)
                            : uni.studentCount,
                        establishedYear: typeof uni.establishedYear === "string"
                            ? parseInt(uni.establishedYear)
                            : uni.establishedYear,
                        estimatedMatchScore: typeof uni.estimatedMatchScore === "string"
                    ...
                    console.error("Failed to parse suggestions:", suggestError, "Raw response length:", suggestResponseText.length);
    ...
    async insertSuggestedUniversities(suggestedUniversities, db) {
        ...
        for (const suggested of suggestedUniversities) {
            try {
                ...
                const existingUniversity = await db
                    .select()
                    .from(universities)
                    .where(and(eq(universities.name, suggested.name), eq(universities.country, suggested.country)))
                    .limit(1);
                if (existingUniversity.length > 0) {
                    console.log(`🔄 University ${suggested.name} already exists, skipping...`);
                ...
                const newUniversity = await db
                    ...
                    description: suggested.description ||
                        suggested.reasoning ||
                        `University recommended by AI matching system. Specializes in ${suggested.specialties?.join(", ") || "various fields"}.`,
                    ...
                    for (const scholarshipData of suggested.scholarships) {
                        try {
                            ...
                            const existingScholarship = await db
                                .select()
                                .from(scholarships)
                                .where(and(eq(scholarships.name, scholarshipData.name), eq(scholarships.provider, scholarshipData.provider || "Unknown")))
                                .limit(1);
                            let scholarship;
                            if (existingScholarship.length > 0) {
                                scholarship = existingScholarship[0];
                                console.log(`🔄 Scholarship ${scholarshipData.name} already exists, linking to university...`);
                            }
                            else {
                                ...
                                const newScholarship = await db
                                    ...
                                    type: scholarshipData.type === "fully-funded"
                                        ? "fully-funded"
                                    ...
                                    deadline: scholarshipData.deadline || "Contact for deadline",
                                    ...
                                    maxRecipients: typeof scholarshipData.maxRecipients === "number"
                            ...
                            console.log(`🔗 Linked scholarship ${scholarshipData.name} to university ${suggested.name}`);
                        }
                        catch (scholarshipError) {
                            console.error(`❌ Failed to process scholarship ${scholarshipData.name}:`, scholarshipError);
                ...
                await this.matchExistingScholarships(newUniversity[0].id, suggested.country, suggested.specialties || [], db);
                insertedUniversities.push(newUniversity[0]);
            }
            catch (error) {
                console.error(`❌ Failed to insert university ${suggested.name}:`, error);
                // Continue with other universities even if one fails
            }
        }
        return insertedUniversities;
    }
    async matchExistingScholarships(universityId, country, specialties, db) {
        try {
            console.log(`🔍 Looking for existing scholarships for country: ${country}, specialties: ${specialties.join(", ")}`);
            ...
            for (const scholarship of matchingScholarships) {
                ...
                const hasMatchingPrograms = scholarship.eligiblePrograms.some((program) => specialties.some((specialty) => specialty.toLowerCase().includes(program.toLowerCase()) ||
                ...
                if (hasMatchingPrograms || scholarship.eligiblePrograms.length === 0) {
                    ...
                    const existingLink = await db
                        ...
                        .where(and(eq(universityScholarships.universityId, universityId), eq(universityScholarships.scholarshipId, scholarship.id)))
                        .limit(1);
                    if (existingLink.length === 0) {
                        ...
                        console.log(`🔗 Matched existing scholarship ${scholarship.name} to AI university`);
                    }
                }
            }
        }
    ...
    async generateTaskRecommendations(profile, favoriteUniversities, favoriteScholarships) {
        try {
            console.log("🎯 Generating task recommendations for user profile...");
            const prompt = this.createTaskRecommendationPrompt(profile, favoriteUniversities, favoriteScholarships);
            const response = await this.genai.models.generateContent({
                model: "gemini-flash-latest",
                contents: prompt,
                config: {
    ...
    createTaskRecommendationPrompt(profile, favoriteUniversities, favoriteScholarships) {
...
Generate 1-5 tasks focused on the student's specific favorite universities and scholarships. Make tasks specific and actionable.`;
    ...
    parseTaskRecommendations(aiResponseText) {
        try {
            ...
            return [
                {
                    ...
                    priority: "MUST",
                    dueDate: "2025-12-01",
                    notes: "Research requirements for target universities and prepare accordingly",
                ...
                },
                {
                    ...
                    priority: "MUST",
                    dueDate: "2025-11-15",
                    notes: "Write compelling personal statement highlighting your goals and experiences",
                    tags: ["essays", "applications"],
                },
            ];
        }
    }
    async generateChatResponse(userMessage, profile, favoriteUniversities, favoriteScholarships, chatHistory) {
        try {
            console.log("🤖 Generating chat response for user message...");
            const prompt = this.createChatPrompt(userMessage, profile, favoriteUniversities, favoriteScholarships, chatHistory);
            const response = await this.genai.models.generateContent({
                model: "gemini-flash-latest",
                contents: prompt,
                config: {
    ...
    createChatPrompt(userMessage, profile, favoriteUniversities, favoriteScholarships, chatHistory) {
        const universityNames = favoriteUniversities.map((u) => u.name).join(", ");
        const scholarshipNames = favoriteScholarships.map((s) => s.name).join(", ");
...
Response:`;
    ...
    parseChatResponse(aiResponseText) {
