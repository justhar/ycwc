...
export class TaskRepository {
    ...
    async update(taskId, userId, updates) {
        ...
        const { id, userId: _, createdAt, updatedAt, subtasks, ...updateData } = updates;
        const result = await db
            .update(tasks)
            .set(updateData)
            .where(and(eq(tasks.id, taskId), eq(tasks.userId, userId)))
     ...
     */
    async findSubtasksByTaskId(taskId) {
        return await db
            .select()
            .from(subtasks)
    ...
    async updateSubtask(subtaskId, updates) {
        // Remove read-only fields
        const { id, taskId, createdAt, ...updateData } = updates;
        const result = await db
            .update(subtasks)
            .set(updateData)
     ...
     */
    async findWithSubtasks(userId) {
        const userTasks = await this.findByUserId(userId);
        const tasksWithSubtasks = await Promise.all(userTasks.map(async (task) => {
