...
export class UniversityRepository {
    ...
    async findAll(params) {
        ...
        if (params?.country) {
            query = query.where(eq(universities.country, params.country));
        }
        if (params?.type) {
            query = query.where(eq(universities.type, params.type));
        }
        if (params?.minRanking) {
            query = query.where(gte(universities.ranking, params.minRanking));
        }
        if (params?.maxRanking) {
            query = query.where(lte(universities.ranking, params.maxRanking));
        }
        // Apply pagination
     ...
     */
    async searchWithFilters(searchTerm, params) {
        const conditions = [ilike(universities.name, `%${searchTerm}%`)];
        if (params.country) {
            conditions.push(eq(universities.country, params.country));
        }
        if (params.type) {
     ...
     * Update university
     */
    async update(universityId, updates) {
     ...
     */
    async isFavorited(userId, universityId) {
        const result = await db
            .select()
            .from(userFavorites)
            .where(and(eq(userFavorites.userId, userId), eq(userFavorites.universityId, universityId)));
     ...
     */
    async count(params) {
        let query = db.select({ count: sql `count(*)` }).from(universities);
        if (params?.country) {
            query = query.where(eq(universities.country, params.country));
        }
        if (params?.type) {
     ...
     */
    async findFavoriteScholarshipsByUserId(userId) {
        return await db
            .select({
     ...
     */
    async findScholarshipsByUniversityId(universityId) {
        return await db
            .select({
            id: scholarships.id,
            ...
            .from(scholarships)
            .innerJoin(universityScholarships, eq(scholarships.id, universityScholarships.scholarshipId))
