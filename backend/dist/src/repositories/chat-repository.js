...
export class ChatRepository {
     ...
     * Update chat
     */
    async update(chatId, userId, updates) {
        // Remove read-only fields
        const { id, userId: _, createdAt, updatedAt, messages, ...updateData } = updates;
        const result = await db
            .update(chats)
            .set(updateData)
            .where(and(eq(chats.id, chatId), eq(chats.userId, userId)))
     ...
     */
    async updateTitleFromMessage(chatId, userId, messageContent) {
