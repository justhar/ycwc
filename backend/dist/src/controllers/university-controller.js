...
export class UniversityController {
    ...
    async getUniversities(c) {
        try {
            ...
            if (search && search.trim().length > 0) {
                const searchResults = await universityService.searchUniversitiesWithFilters(search, {
     ...
     */
    async getUniversityScholarships(c) {
        try {
            const universityId = c.req.param("id");
            const scholarships = await universityService.getUniversityScholarships(universityId);
     ...
     */
    async searchUniversities(c) {
        try {
            const searchTerm = c.req.query("q") || "";
            const limit = c.req.query("limit") ? parseInt(c.req.query("limit")) : 10;
            const universities = await universityService.searchUniversities(searchTerm, limit);
