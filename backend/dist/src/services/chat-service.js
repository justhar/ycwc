...
class ChatService {
    ...
    async addMessage(chatId, userId, messageData) {
        ...
        // Store user message
        const userMessage = await chatRepository.createMessage(chatId, messageData.role, messageData.content);
        ...
        if (messageData.role === "user") {
            try {
                ...
                const aiResponse = await aiService.generateChatResponse(messageData.content, userProfile || {}, favoriteUniversities || [], favoriteScholarships || [], chatMessages);
                // Store AI response
                const assistantMessage = await chatRepository.createMessage(chatId, "assistant", aiResponse.response);
