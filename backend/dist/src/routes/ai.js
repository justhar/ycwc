...
const getLocalizedMessage = (c, key) => {
    ...
    try {
        ...
        try {
            ...
            if (cvFile) {
                ...
                analysisResult = await aiService.analyzeCVFile(Buffer.from(fileBuffer), cvFile.type);
            }
            else {
...
ai.post("/match", authMiddleware, async (c) => {
    try {
        ...
        const matchingResult = await aiService.matchUniversities(profile, allUniversities);
        if (!matchingResult.success) {
            return c.json({
                success: false,
                error: matchingResult.error || getLocalizedMessage(c, "aiMatchingFailed"),
            }, 500);
        }
        // Handle suggested universities - insert them into database with deduplication
        let insertedSuggestions = [];
        if (matchingResult.suggestedUniversities &&
            matchingResult.suggestedUniversities.length > 0) {
            try {
                insertedSuggestions = await aiService.insertSuggestedUniversities(matchingResult.suggestedUniversities, db);
            }
            catch (error) {
        ...
        return c.json({
            ...
            data: {
                ...
                suggestedUniversities: insertedSuggestions.length > 0
                    ? insertedSuggestions
